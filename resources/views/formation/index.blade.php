<x-app-layout>
    ...
    <div class="bg-gradient-to-br from-[#1A1A2E] to-[#2D6A4F] text-white py-14 px-4">
        <div class="max-w-5xl mx-auto">
            <h1 class="text-3xl sm:text-4xl font-display font-bold mb-3 flex items-center gap-3">
                <x-icon name="graduation" class="w-9 h-9 sm:w-10 sm:h-10 shrink-0" />
                {{ __('formation.title') }}
            </h1>
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        ...
        @if($formations->isEmpty())
            <div class="text-center py-16 text-gray-400">
                <x-icon name="inbox" class="w-14 h-14 mx-auto mb-3 text-gray-300" />
            ...
            <a href="{{ route('formation.show', $formation->slug) }}"
               class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden group flex flex-col block">
                @if($formation->thumbnail)
                    <img src="{{ asset('storage/'.$formation->thumbnail) }}" class="w-full h-40 object-cover group-hover:scale-105 transition-transform duration-300" alt="{{ $formation->title }}">
                @else
                    <x-formation-cover-placeholder class="w-full h-40 group-hover:scale-105 transition-transform duration-300" />
                @endif
                <div class="p-4 flex-1 flex flex-col">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @if($formation->is_online)
                            <span class="inline-flex items-center gap-1 text-xs text-blue-600 font-medium">
                                <x-icon name="globe" class="w-3.5 h-3.5" /> {{ __('formation.online') }}
                            </span>
                        @endif
                    </div>
                    ...
                    @if($formation->organizer)
                        <p class="text-xs text-gray-500 mb-1 flex items-center gap-1.5">
                            <x-icon name="building" class="w-3.5 h-3.5 shrink-0" /> {{ $formation->organizer }}
                        </p>
                    @endif
                    @if($formation->start_date)
                        <p class="text-xs text-gray-500 mb-1 flex items-center gap-1.5">
                            <x-icon name="calendar" class="w-3.5 h-3.5 shrink-0" /> {{ $formation->start_date->translatedFormat('d F Y') }}
                        </p>
                    @endif
                    @if($formation->country)
                        <p class="text-xs text-gray-500 flex items-center gap-1.5">
                            <x-icon name="location" class="w-3.5 h-3.5 shrink-0" /> {{ $formation->country }}{{ $formation->location ? ' · '.$formation->location : '' }}
                        </p>
                    @endif
